...
async function scrapeRoster(url) {
  try {
    ...
    rosterList.find('li').each((index, element) => {
      ...
      const position = $(element).find('div.sidearm-roster-player-position span.text-bold').text().trim();
      ...
      const year = $(element).find('span.sidearm-roster-player-academic-year:not(.hide-on-large)').text().trim();
